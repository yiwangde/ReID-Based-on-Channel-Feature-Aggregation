...
# Define the ResNeFt50-based Model
class resneft50(nn.Module):

    def __init__(self, class_num, droprate=0.5, stride=2):
        super(resneft50, self).__init__()
    ...
    def forward(self, x):
        ...
        P3, P4, P5, P6, P7 = self.model.fpn([x2, x3, x4])
        # print('p7.shape',P7.shape)
        # print('p3.shape',P3.shape)

        # x = P3i
        g3 = F.adaptive_avg_pool2d(P3, 1)
        # print('g3', g3.shape)
        g4 = F.adaptive_avg_pool2d(P4, 1)
        # print('g4', g4.shape)
        g5 = F.adaptive_avg_pool2d(P5, 1)
        # print('g5', g5.shape)
        g6 = F.adaptive_avg_pool2d(P6, 1)
        # print('g6', g6.shape)
        g7 = F.adaptive_avg_pool2d(P7, 1)
        # print('g7', g7.shape)

        # x = torch.cat((P3, P4, P5, P6, P7), dim=1)
        # print(x.shape)
        # x = torch.cat([self.model.classificationModel(feature) for feature in features], dim=1)
        # print('shape',x.shape)
        
        x = torch.cat([g3, g4, g5, g6, g7], dim=1).contiguous()
        # x = P7
...
# Define the ResNeSt50-based Model
class ft_resnest50(nn.Module):

    def __init__(self, class_num, droprate=0.5, stride=2):
        super(ft_resnest50, self).__init__()
...
# Define the ResNeSt269-based Model
class ft_resnest269(nn.Module):

    def __init__(self, class_num, droprate=0.5, stride=2):
        super(ft_resnest269, self).__init__()
        model_ft = resnest.resnest269(pretrained=True)
        # avg pooling to global pooling
        if stride == 1:
            model_ft.layer4[0].downsample[0].stride = (1,1)
            model_ft.layer4[0].conv2.stride = (1,1)
